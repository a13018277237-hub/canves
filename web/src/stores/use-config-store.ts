...
import { getPluginTemplates } from "@/services/api/model-plugin";
...
/**
 * Resolve the call script for a model:
 * - returns the user-configured script if set
 * - otherwise falls back to the standard template matching the channel's apiFormat
 *   (OpenAI template for openai/ark, Gemini template for gemini)
 * - returns empty string only when the model/capability cannot be resolved,
 *   in which case callers fall back to their hardcoded default call.
 */
export function resolveModelScript(config: AiConfig, value: string) {
    const found = findChannelModel(config, value);
    const userScript = found?.model.script?.trim();
    if (userScript) return userScript;

    // 未设置调用脚本时，使用对应能力与渠道格式的标准模板作为调用脚本
    const capability = found?.model.capability;
    if (!capability) return "";
    const apiFormat = found.channel?.apiFormat || config.apiFormat;
    const templates = getPluginTemplates()[capability] || [];
    // 模板顺序：[0] = OpenAI, [1] = Gemini
    const template = apiFormat === "gemini" ? templates[1] || templates[0] : templates[0];
