...
if (images.length === 0) {
  ...
  const data = await request({
    ...
    data: {
      ...
      n: params.count,
      size: params.size
      // ${i18n.t("modelPlugin.templates.gptImageDefaultB64Generations")}
    },
  });
...
// ${i18n.t("modelPlugin.templates.gptImageDefaultB64Edits")}
...
const edited = await request({
  ...
  headers: {
    Authorization: \`Bearer \${apiKey}\`
    // ${i18n.t("modelPlugin.templates.formDataHeader")}
  },
  data: form,
});
