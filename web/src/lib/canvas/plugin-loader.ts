...
async function loadLocalPlugins() {
    let urls: unknown;
    try {
        // 用 BASE_URL 拼接，适配 Vite 默认根路径与 Electron 打包后的相对路径（file://）
    ...
    await Promise.all(
        urls.map(async (rawUrl: string) => {
            // 本地 manifest 里的绝对路径（/plugins/...）按 BASE_URL 重新拼接，适配 file:// 等非根部署
            const url = rawUrl.startsWith("/") ? `${import.meta.env.BASE_URL}${rawUrl.slice(1)}` : rawUrl;
            try {
